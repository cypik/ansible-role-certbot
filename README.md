...
**IMPORTANT:** Since the `master` branch used in `source` varies based on new modifications, we suggest that you use the release versions [here](https://github.com/cypik/ansible-role-certbot/releases).
...
```console
  $ ansible-galaxy install cypik.ansible_role_certbot
```
